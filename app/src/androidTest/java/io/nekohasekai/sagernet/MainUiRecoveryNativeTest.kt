                ...
                scenario.onActivity {
                    ...
                    if (page == R.id.nav_settings) {
                        ...
                        for (key in listOf("uiEditApps", "remoteDns", "mixedPort",
                            "globalAllowInsecure", "mtu", "domain_strategy_for_remote", "enableDnsRouting")) {
                            assertTrue("Setting must be exposed without expansion: $key",
                                adapter.getPreferenceAdapterPosition(key) >= 0)
                        }
                        for (key in listOf("uiDetailedDiagnostics", "uiManagedSettings", "uiProxyDetails", "uiConnectionDetails", "uiRoutingDetails", "uiDomainDetails")) {
    ...
    @Test fun settingsDetailsRetainOverridesAcrossModesAndRecreation() {
        ...
        withMain { scenario ->
            ...
            scenario.onActivity { activity ->
                val settings = activity.supportFragmentManager.findFragmentById(R.id.settings)
                    as io.nekohasekai.sagernet.ui.SettingsPreferenceFragment
                assertNull(settings.findPreference<androidx.preference.Preference>("uiProxyDetails"))
                assertNotNull(settings.findPreference<androidx.preference.Preference>("domain_strategy_for_server"))
            }
        }
    }
