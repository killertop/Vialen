...
def main():
 p=argparse.ArgumentParser();p.add_argument('--target-file',required=True);p.add_argument('--artifacts',type=pathlib.Path,required=True);p.add_argument('--output',type=pathlib.Path,required=True);p.add_argument('--plan',choices=['aa','L','L-list','B-gc'],required=True);a=p.parse_args()
