...
导入窗口现在要求数据库提交行数、有效选中节点、UI 行数和监听通知行数同时满足预期。L0 的历史接口逐条通知，因此预期批量回调为 0；L1/B1 的批量接口预期为 1。宿主采集器也按固定历史源码 SHA 复核结果，缺失字段不能算成功。

列表/导入窗口要求目标主界面在整个窗口保持前台；丢失前台会让该轮失败。该断言在 2026-09-16 的部分列表采集停止后加入，不能追溯给旧窗口补上前台证据。

B1 的 GC 入口等待原生门控空闲并核对强制 GC 周期确实增加；L0/L1 的旧实现没有同等完成信号，入口明确拒绝旧版 GC 计量。此改动只在测量叠加层，仍未覆盖真实系统内存回调和长期内存。
