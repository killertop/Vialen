> 当前结论与有效待办请先查看 [当前状态](../docs/current-status.md)。下文按历史批次保留，不把已被后续结果取代的阻塞重新算作待办。
...
原路径直接将剪贴板送入节点解析器；原 SubscriptionFoundException 捕获已无对应抛出方，带路径和查询参数的网页订阅因此无法进入确认流程。新增 ClipboardSubscription：只检查单个 HTTP/HTTPS 地址，先用实际核心识别合法代理节点，其他网页地址复用 SubscriptionLink 校验，再通过 Uri.Builder 的 url 参数进入现有订阅确认界面。完整保留编码、重复参数及无值标记；不根据机场域名硬编码，不读取网络内容。订阅分支不受普通分组不存在影响，节点分支继续使用点击时的目标分组快照。纯 HTTP(S) 代理端点仍作为节点处理，地址语义不明确时可使用添加订阅入口。

新增 4 项真实核心解析回归，覆盖合成 token/编码/重复参数/无值标记、HTTP(S) 代理兼容、多行及非网页内容、非法端口与认证地址拒绝。全量 358 tests / 0 failures / 0 errors / 0 skipped；Lint 与 Release 构建通过。首次全量通过后，补充目标分组快照保护，再次完成同一全量及构建；不删除失败测试、不降低断言。

版本 2.1.5 / VERSION_CODE=103 / ARM64=515。使用既有正式签名，证书与 2.1.4 相同；源码及 APK 公开内容检查、16 KB 对齐通过。正式包在真机保留数据升级成功并回读版本。尝试用合成包装链接检查确认页面时设备处于锁屏，未完成真机剪贴板点击及确认页视觉验收；没有解锁设备或确认导入、访问用户提供的订阅、启动 VPN。原始日志和截图严格保留仓库外，未写入真实地址和凭据。

只做本地提交和正式包安装，不推送、不发布。构建后将本轮可再生缓存移入系统垃圾篓，保留必要 AAR；没有重新安装隔离包。

## 订阅确认与更新结果弹窗精简（2.1.6）

起点 main / `2d62447`，工作区干净。按用户要求移除订阅确认中的“不可信”警告；未命名订阅只显示来源主机，避免完整查询令牌占据弹窗。显示变化不改动传给订阅流程的原始链接。确认页保留导入/取消，增加链接图标。

更新结果沿用项目圆角 Material 弹窗，增加完成图标与简短标题，首先显示分组及新增、更新、移除、重复数量；“查看详情/收起详情”在同一可滚动消息区切换，保留原节点变更明细。“完成”关闭弹窗。移除重复成功 Snackbar 和人为的一秒等待；无变化反馈及失败处理保持原有逻辑。不改变订阅下载、数据库提交和取消行为。

执行 `:app:testDebugUnitTest :app:lintDebug :app:assembleRelease` 成功：358 tests / 0 failures / 0 errors / 0 skipped。此次为展示改动，未增加镜像实现测试。源码和正式 APK 公开内容检查、16 KB 对齐均通过；原正式证书与前版一致。版本 2.1.6 / VERSION_CODE=104 / ARM64=520，真机保留数据升级成功并回读版本。

正式包使用合成链接进入订阅确认，通过 UI 层级回读确认标题、仅来源域名、警告及令牌不显示，随后点击取消。未确认导入、请求真实订阅或启动 VPN；节点更新结果弹窗的实际下载后展示及长详情滚动尚未真机验收。原始构建和设备证据保留仓库外。只做本地提交和安装，不推送、不发布。

## 回滚订阅结果摘要交互（2.1.7）

按用户要求，订阅更新结果恢复 2.1.5 的单层明细弹窗，移除数量摘要及“查看详情/收起详情”切换和对应资源。保留订阅导入确认文案精简、隐藏查询令牌，以及剪贴板订阅识别修复。未更改下载或数据库逻辑。

起点 main / `5bb9e54`，工作区干净。执行 `:app:testDebugUnitTest :app:lintDebug :app:assembleRelease` 成功，358 tests / 0 failures / 0 errors / 0 skipped；源码/正式 APK 公开内容扫描和 16 KB 对齐通过。版本 2.1.7 / VERSION_CODE=105 / ARM64=525，正式签名与前版一致，真机保留数据升级成功并回读版本。本次未触发真实订阅更新以制造结果弹窗；明细展示恢复依据原文件逐字回读，不冒充本轮视觉验收。原始产物和日志留在仓库外；本地提交，不推送、不发布。

## 当前待办整理与第四批续验

以 2.1.7 / `2077e05` 为生产基线，新增 [当前状态](../docs/current-status.md)，将已关闭问题、未完成测量、平台边界和用户取消项分开；保留历史失败及后续关闭记录。

本轮重新验证三份基线 APK/历史源码与既有记录一致，完成新一轮 6 个正式 A/A 窗口及 12 次预热。两次列表计划获得部分对照：第一次因结果文件缺失中止，随后观察到 ADB 离线；恢复后的独立诊断通过。固定的一次重采得到 1,000/10,000 节点各一对完整窗口，下一对达到 40.2℃，依原阈值排除并停止。未达到 10 对，不得报告版本提速或省电结论；详见 [续验报告](../benchmarks/batch4/resume-report.md)。

工具增加单独 L-list 计划与种子/环境无效即停止防护；8 项宿主测试通过，源码公开内容检查通过。未改生产算法、版本、网络或显示设置。测量包已卸载并回读仅保留正式包，正式数据和连接未操作。未重新执行全量 JVM/Lint：此次只改 Python 测量工具及文档/脱敏数据，沿用已通过的 2.1.7 生产基线，不冒充本轮生产代码回归。原始证据仓库外保留，本轮只本地提交、不推送。

## 订阅失败分类与中文提示（2.1.8）

本轮起点 main / `ddca114`，工作区干净。原生统一 TEMPORARY 提示不能区分 DNS/超时/连接/HTTP 原因；直连竞争路径还把 HTTP 状态变为普通 error，可能丢失拒绝与重试语义。现保留类型并以稳定传输顺序优先使用实际 HTTP 拒绝，区分 TIMEOUT、DNS_FAILED、NETWORK_ERROR、RATE_LIMITED、SERVER_ERROR、ACCESS_DENIED、NOT_FOUND 等稳定代码。只映射固定中文，不显示原始链接、响应正文或异常文本。401/403/404/410 等仍按永久失败处理；408/425/429/5xx、网络失败仍允许受限重试。直连响应头期限明确传播为超时，而不是用户取消；用户取消继续独立传播。

新增 Kotlin 提示/重试/取消回归，原生回环测试同时覆盖普通和直连 HTTP 的各类状态及失败正文不泄露；额外 race 用例确认直连响应头超时不误判为取消。全量 libcore 普通/race 通过，新增响应头用例单独 race 通过；全量 Android JVM/Room 361 tests / 0 failures / 0 errors / 0 skipped，Lint、Release 构建通过。初次编译因删除状态格式化后遗留 fmt 导入失败，移除未使用导入后通过，未删除测试或降低断言。

使用 `scripts/build-private-android.sh` 从中性路径重建原生 AAR。2.1.8 / VERSION_CODE=106 / ARM64=530；正式签名与前版一致，源码/APK 公开内容检查、16 KB 对齐通过。错误分支使用合成回环输入，未读取或请求个人订阅；未在正式手机逐一制造 DNS/证书/限流等故障。本轮不推送或发布。

正式 2.1.8 已在真机保留数据升级成功，回读版本和包名一致，仅保留正式包。本次新增自然冷却门控与 4 项确定时序测试，Python 工具共 12 项通过。门控要求连续两次不高于 37℃且热状态正常，每处最多 5 分钟、全计划最多 30 分钟；不改变供电、网络或显示设置。真机就绪检查持续 272.4 秒、采样 10 次，从 38.9℃降至 37.9℃；下一次等待将超出预算，按约定结束。未达到开测条件，未安装测量包或启动新性能窗口，完整对照仍未完成。原始环境和安装日志保留仓库外。
